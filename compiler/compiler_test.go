	...
	tests := []compilerTestCase{
		{
			...
			input: "false", expectedConstants: []interface{}{},
			expectedInstructions: []code.Instructions{
	...
	tests := []compilerTestCase{
		...
		},
		{
			...
			expectedInstructions: []code.Instructions{
				...
				code.Make(code.OpTrue),
				// 0001
				code.Make(code.OpJumpNotTruthy, 10), // 0004
				code.Make(code.OpConstant, 0),
				// 0007
...
func TestStringExprs(t *testing.T) {
	tests := []compilerTestCase{
		{
			input:             `"monkey"`,
...
func TestCompilerScopes(t *testing.T) {
	...
	globalSymT := compiler.symbolTable
	...
	if compiler.symbolTable.Outer != globalSymT {
	...
	if compiler.symbolTable != globalSymT {
	...
	previous := compiler.scopes[compiler.scopeIndex].lastToLastInstruction
...
func TestLetStatementScopes(t *testing.T) {
	tests := []compilerTestCase{
		{input: `let a=1; let b=2; let c=a+b; c`,
			expectedConstants: []interface{}{1, 2},
			expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpSetGlobal, 0),
...
func TestIndexExpressions(t *testing.T) {
	tests := []compilerTestCase{
		...
		{
			input: "{1: 2}[2 - 1]", expectedConstants: []interface{}{1, 2, 2, 1}, expectedInstructions: []code.Instructions{
				code.Make(code.OpConstant, 0),
				code.Make(code.OpConstant, 1),
